...
export default {
  ...
  created() {
    ...
    axios(`https://api.premid.app/v2/credits/${Vue.$route.params.userid}`)
      .then(res => {
        ...
        axios(`https://api.premid.app/v2/presences`)
          .then(res => {
            var foreach = res.data.map(presence => {
              if (presence.metadata.author.id == Vue.$data.userdata.userId)
                Vue.$data.userpresences.push(presence.metadata);
            });
