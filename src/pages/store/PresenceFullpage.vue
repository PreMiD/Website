      ...
      <div v-if="!$root.isProcessing" class="fullpresence-container">
        <div class="fullpresence__header" :style="`background-image: url('${presenceData.thumbnail}')`">
          ...
          <div class="header__buttons">
            <button v-if="!isInstalled && this.$root.extensionInstalled && typeof presenceData.button == 'undefined'"
              class="button button_light" v-on:click="sendPresence(presenceData.service)">
              <span class="icon">
                <i class="fas fa-plus"></i>
              </span>
              {{ $t('store.card.presence.add') }}
            </button>
            <button v-if="isInstalled" class="button button_black" v-on:click="removePresence(presenceData.service)">
              <span class="icon">
            ...
            <a class="button button_black"
              :href="`https://github.com/PreMiD/Presences/tree/master/${$route.params.presenceName}`" target="_blank">
              <span class="icon">
        ...
        <div class="fullpresence__content">
          <div class="content__description">
            ...
            <div class="description-container" v-if="!presenceData.fullDescription && presenceData.description"
              v-html="getPresenceDescription()" />
            <div class="description-container" v-if="presenceData.fullDescription"
            ...
            <ul class="info__sections">
              <li v-if="presenceData.author">
                <p>
                  ...
                  <router-link v-if="presenceAuthor" class="author-name" :style="`color: ${presenceAuthor.roleColor};`"
                    :to="`/users/${presenceAuthor.userID}`">
                    <img v-if="presenceAuthor.avatar" :src="presenceAuthor.avatar" class="author-avatar" />
              ...
              <li v-if="presenceData.tags">
                ...
                <div class="presence-tags">
                  <a v-bind:key="tag" v-for="(tag) of presenceData.tags"
  ...
  export default {
    ...
    methods: {
      ...
      getPresenceDescription() {
        if (this.$data.presenceData.description[this.$root.getCurrentLanguage()]) {
    ...
    computed: {
      ...
      presenceGradientColor() {
        return tinycolor(this.$data.presenceData.color).darken(45).toHexString();
      }
    }
  };
