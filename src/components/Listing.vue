<template>
  <transition name="card-animation" mode="out-in">
    <div class="presence-container__item">
      <div class="store-card" @mouseover="card_hovered = true" @mouseleave="card_hovered = false"
        :style="`background-image: url('${presence.thumbnail}')`">
          ...
          <div class="store-card__service">
            <h2>
              ...
              <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"
                :title="$t('store.cards.verified')" v-tippy class="store-card__verified" :style="`fill: ${presence.color}`">
                <g>
            ...
            <transition name="card-animation" mode="out-in">
              <div key="desc" v-if="!card_hovered">
                <p v-if="!card_hovered" class="store-card__desc">{{ presence.description }}</p>
              </div>
              <div key="buttons" v-if="card_hovered">
                <div v-if="this.$parent.extension_installed && typeof presence.button == 'undefined'"
                  ...
                  <button v-if="!isInstalled" class="button button_light" v-on:click="sendPresence(presence.service)">
                    ...
                    </span> {{ $t('store.card.presence.add') }}
                  </button>
                  <button v-if="isInstalled" class="button button_black" v-on:click="removePresence(presence.service)">
                    <span class="icon">
                      <i class="fas fa-minus"></i>
                    </span> {{ $t('store.card.presence.remove') }}
                  </button>
                </div>
